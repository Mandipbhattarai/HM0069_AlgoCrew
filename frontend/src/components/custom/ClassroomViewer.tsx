...
export default function ClassroomViewer() {
  ...
  const handleFeedback = async (assignment: Assignment) => {
    ...
    try {
      ...
      if (
        !submission ||
        !submission.assignmentSubmission?.attachments?.length
      ) {
      ...
      try {
        ...
        const analysisResponse = await axios.post(
          "http://127.0.0.1:8000/analyze_document",
          {
        ...
        // Navigate to feedback page with ALL necessary information
        navigator("/feedback", {
          state: {
            assignment: {
            ...
            document: analysisResponse.data.document,
            feedback: analysisResponse.data.analysis,
            fullResponse: analysisResponse.data,
          },
        });
      } catch (analysisError) {
